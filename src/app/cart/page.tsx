...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <Container className="flex min-h-[60vh] flex-col items-center justify-center gap-5 py-20 text-center">
        <BagIcon className="h-12 w-12 text-stone" />
      ...
      <div className="mt-10 grid gap-12 lg:grid-cols-[1.6fr_1fr]">
        ...
        <div>
          <ul className="divide-y divide-line border-t border-line">
            {items.map((item) => (
              <li key={`${item.productHandle}-${item.size}`} className="flex gap-5 py-6">
                <Link
                  href={`/products/${item.productHandle}`}
                  className="relative h-32 w-26 shrink-0 overflow-hidden bg-mist"
                >
                  ...
                  <div className="mt-auto flex items-center justify-between pt-4">
                    <div className="flex items-center border border-line">
                      <button
                        ...
                        className="grid h-9 w-9 place-items-center hover:bg-ink/5"
          ...
          <div className="mt-6 flex justify-between">
            ...
            <button onClick={clear} className="text-sm text-stone hover:text-ink">
              Clear cart
            </button>
          </div>
        </div>
        ...
        {/* Summary */}
        <aside className="h-fit border border-line bg-paper p-7 lg:sticky lg:top-28">
