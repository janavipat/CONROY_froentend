...
/**
 * Phone number + OTP (WhatsApp) — sign-in and sign-up both authenticate by
 * mobile number, matching the account's identity in the DB. No email is
 * collected here at all (that's the checkout form's job) — email + password
 * auth (EmailAuthForm) is commented out in LoginExperience.tsx in favor of
 * this.
 */
export function PhoneOtpAuth({ mode = "signin" }: { mode?: AuthMode }) {
  ...
  // Focus the phone field AFTER hydration (avoids dropping the first
  ...
  async function handleSend() {
    ...
    setSending(true);
    const res = await sendOtp(e164, remember, mode);
    setSending(false);
    if (res.error) {
    ...
    <div>
        ...
        {step === "phone" ? (
          ...
          >
            <p className="text-sm leading-relaxed text-ink-soft">
              {isSignup
                ? "Create your account — we'll send a one-time verification code to your mobile number."
      ...
      {!isConfigured && (
        <p className="mt-6 border border-dashed border-line bg-mist/60 px-3 py-2 text-center text-xs text-stone">
          ...
          WhatsApp in the backend for live OTP.
        </p>
      )}
    </div>
  );
